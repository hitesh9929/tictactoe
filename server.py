    ...
    while failed:
        try:
            ...
            if validate_input(x, y, conn):
                ...
                failed = 0  
                send_common_msg("Board")
                send_common_msg(str([x,y,currentPlayer]))
...
def check_rows():
    ...
    result = 0
    for i in range(3):
        if matrix[i][0] == matrix[i][1] and matrix[i][1] == matrix[i][2] and matrix[i][0]!=0:
            result = matrix[i][0]
            if result != 0:
                break
    return result
...
def check_columns():
    ...
    result = 0
    for i in range(3):
        if matrix[0][i] == matrix[1][i] and matrix[1][i] == matrix[2][i] and matrix[0][i]!=0:
            result = matrix[0][i]
            if result != 0:
                break
    return result
...
def check_diagonals():
    ...
    result = 0
    if matrix[0][0] == matrix[1][1] and matrix[1][1] == matrix[2][2] and matrix[0][0]!=0:
        result = matrix[0][0]
    elif matrix[0][2] == matrix[1][1] and matrix[1][1] == matrix[2][0] and matrix[0][0]!=0:
        result = matrix[0][2]
    return result
...
def start_game():
    ...
    if result == 1:
        lastmsg = f"Player one  {playerName[0]} is the winner"
    elif result == 2:
        lastmsg = f"Player two  {playerName[1]} is the winner"
    else:
        lastmsg = "Draw"
